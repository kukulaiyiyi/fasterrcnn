...
def concat_box_prediction_layers(box_cls, box_regression):
    ...
    的tensor排列顺序以及shape进行调整 [B,N/(4*N),H,W]-> [N, -1, C]
    Args:
...
class RegionProposalNetwork(torch.nn.Module):
    ...
    def filter_proposals(self, proposals, objectness, image_shapes, num_anchors_per_level):
        ...
        # tensor.detach() 分离复制tensor,在改变新tensor时不会影响原变量的梯度计算
    ...
    def forward(self,
        ...
        features = list(features.values())   #B*1280*25*38

        # 计算每个预测特征层上的预测目标概率和bboxes regression参数
        # objectness和pred_bbox_deltas都是list   objections(4*15*25*38) bbox_deltas(4*60*25*38)
        objectness, pred_bbox_deltas = self.head(features)

        # 生成一个batch图像的所有anchors信息,list(tensor)元素个数等于batch_size
        anchors = self.anchor_generator(images, features)       #14250 anchor per image
